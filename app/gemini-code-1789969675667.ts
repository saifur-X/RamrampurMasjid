...
export default function AdminDashboard() {
  ...
  // কনফার্মেশন অ্যানিমেটেড পপআপ মডাল
  ...
  // ইনস্ট্যান্ট রিয়েলটাইম সার্চ ফিল্টারিং (নাম, ফোন বা পরিচয় দিয়ে)
  ...
  // নতুন পরিবার তৈরি অথবা তথ্য সংশোধন
  async function handleFamilySubmit(e: React.FormEvent) {
    ...
    if (editingFamilyId) {
      ...
      resetFamilyForm();
      setMsg('নতুন পরিবার এবং বাৎসরিক ১২ মাসের ফি তালিকা তৈরি হয়েছে!');
      loadAllData();
    }
    setLoading(false);
  }
  ...
  // পরিবার ও তার সমস্ত ফি রেকর্ড স্থায়ীভাবে ডিলিট
  function handleDeleteFamily(fam: any) {
    setConfirmModal({
      isOpen: true,
      title: 'পরিবার ডিলিট নিশ্চিতকরণ',
      description: `আপনি কি নিশ্চিত যে ${fam.head_name} সাহেবের পরিবার এবং তাদের সকল ফি রেকর্ড স্থায়ীভাবে মুছে ফেলতে চান?`,
      action: async () => {
        setLoading(true);
  ...
  // একক পরিবারের মাসিক ফি হার এডিট করা
  ...
  // শুধুমাত্র টাকা দেওয়া পরিবারের ফি ম্যানুয়ালি এককালীন Paid করা
  function triggerYearlyPaidModal(fam: any, unpaidList: any[]) {
    ...
    setConfirmModal({
      ...
      description: `${fam.head_name} সাহেব মোট বকেয়া ফি ₹${totalDue}/- জমা দিয়েছেন? শুধুমাত্র তার পরিবারকে পরিশোধিত করতে চান?`,
      action: async () => {
        setLoading(true);
        setMsg('');
        ...
        if (rpcError) {
          ...
          if (directError) {
            setMsg(`ফি আপডেট ব্যর্থ: ${directError.message || rpcError?.message}`);
            setLoading(false);
            setConfirmModal(prev => ({ ...prev, isOpen: false }));
            return;
          }
  ...
  // লেটারহেড সম্বলিত অফিসিয়াল রসিদ ডাউনলোড
  ...
  // ইউনিভার্সাল ফি বৃদ্ধি (সবার ফি সমহারে বাড়ানো)
  function handleUniversalFeeIncrease(e: React.FormEvent) {
    ...
    setConfirmModal({
      ...
      description: `আপনি কি নিশ্চিত যে সকল পরিবারের বকেয়া মাসিক ফি ${incType === 'fixed' ? `₹${val} টাকা` : `${val}%`} বৃদ্ধি করতে চান?`,
      action: async () => {
        setLoading(true);
        setMsg('');
        ...
        if (rpcErr) {
          try {
            ...
            setMsg('ফি বৃদ্ধি করতে সমস্যা হয়েছে।');
            setLoading(false);
            setConfirmModal(prev => ({ ...prev, isOpen: false }));
            return;
          }
        ...
        setIncValue('');
        setConfirmModal(prev => ({ ...prev, isOpen: false }));
        setMsg(`সফলভাবে সকল পরিবারের ফি ${incType === 'fixed' ? `₹${val} টাকা` : `${val}%`} বাড়ানো হয়েছে!`);
        await loadAllData();
      }
    });
  }

  // নোটিস, ফান্ড ও প্রোফাইল ফাংশনসমূহ
  ...
  async function handleSaveMosqueProfile(e: React.FormEvent) {
    ...
    if (!res?.error) {
      setMsg('মসজিদ প্রোফাইল ও লেটারহেড সংরক্ষিত হয়েছে!');
      loadAllData();
    } else {
      setMsg('প্রোফাইল সংরক্ষণ করতে সমস্যা হয়েছে।');
    }
    setLoading(false);
  }
      ...
      <main className="max-w-4xl mx-auto p-4 flex flex-col gap-6">
        {msg && (
          <div className={`p-3 rounded-xl text-xs font-semibold text-center border ${
            msg.includes('সমস্যা') || msg.includes('ব্যর্থ') ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
          }`}>
            {msg}
          </div>
        )}

        {/* ট্যাব ১: বাৎসরিক ফি ও রসিদ */}
        {activeTab === 'fees' && (
          <div className="flex flex-col gap-5">
            ...
            <section className="bg-gradient-to-r from-emerald-800 to-teal-800 text-white p-5 rounded-2xl shadow-sm">
              <h2 className="text-sm font-bold flex items-center gap-2 mb-1">
                ...
                ইউনিভার্সাল ফি বৃদ্ধি (সবার ব্যক্তিগত ফি একসাথে সমহারে বাড়ান)
              </h2>
              <p className="text-xs text-emerald-200 mb-3">
                যার ৫০ তার সাথে যোগ হবে, যার ২০০ তার সাথে যোগ হবে। ১ ক্লিকে সম্পূর্ণ ডেটাবেস আপডেট হবে।
              </p>
        ...
        {/* ট্যাব ২: পরিবার রেজিস্টার, সার্চ, এডিট ও ডিলিট */}
        {activeTab === 'families' && (
          <>
        ...
        {/* ট্যাব ৫: মসজিদ প্রোফাইল ও লেটারহেড */}
        {activeTab === 'settings' && (
          <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h2 className="text-base font-bold flex items-center gap-2 mb-4 text-slate-800">
