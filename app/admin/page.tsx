...
export default function AdminDashboard() {
  ...
  const [selectedTargetYear, setSelectedTargetYear] = useState('১৪৩১-১৪৩২');
  const [scopeMode, setScopeMode] = useState<'single_year' | 'permanent'>('permanent');
  ...
  // Real-time Instant Search (Name, Phone, Aadhaar)
  ...
  async function handleFamilySubmit(e: React.FormEvent) {
    ...
    if (editingFamilyId) {
      ...
      if (!error) {
        setMsg('Poribar-er tothyo shofolbhabe shongshodhon kora hoyeche!');
        resetFamilyForm();
        loadAllData();
      } else {
        setMsg('Poribar shongshodhon korte shomoshya hoyeche.');
      }
    } else {
      const baseAmt = parseFloat(monthlyFee) || 100.00;
      const { data: newFamily, error } = await supabase.from('families').insert([
        { head_name: headName, phone: cleanPhone, aadhaar_masked: aadhaar, base_monthly_fee: baseAmt }
      ]).select().single();

      if (error) {
        setMsg('Poribar jog korte shomoshya hoyeche ba number-ti age thekei ache.');
        setLoading(false);
        return;
      }
      ...
      resetFamilyForm();
      setMsg('Notun poribar ebong batsorik 12 mash-er fee toiri hoyeche!');
      loadAllData();
    }
    setLoading(false);
  }
  ...
  function handleDeleteFamily(fam: any) {
    setConfirmModal({
      isOpen: true,
      title: 'Poribar Delete Nishchitkoron',
      description: `Apni ki nishchit je ${fam.head_name} shaheb-er poribar ebong tader shob fee record muche phelte chan?`,
      action: async () => {
        setLoading(true);
        ...
        if (!error) {
          setMsg(`${fam.head_name} shaheb-er poribar shofolbhabe delete kora hoyeche!`);
          loadAllData();
        } else {
          setMsg('Poribar delete korte shomoshya hoyeche.');
        }
      }
    });
  }
  ...
  async function handleEditFamilyFee(fam: any, currentFee: number) {
    const newAmountStr = prompt(`${fam.head_name} shaheb-er jonno notun mashik fee poriman likhun:`, currentFee.toString());
    ...
    await supabase.from('families').update({ base_monthly_fee: newAmount }).eq('id', fam.id);

    setLoading(false);

    if (!error) {
      setMsg(`${fam.head_name} shaheb-er bokea fee shofolbhabe proti mashe ₹${newAmount} kora hoyeche!`);
      loadAllData();
    } else {
      setMsg('Fee shongshodhon korte shomoshya hoyeche.');
    }
  }

  // Manually Paid for the single family
    ...
    setConfirmModal({
      ...
      action: async () => {
        ...
        if (rpcError) {
          ...
          if (directError) {
            setMsg(`Fee update failed: ${directError.message || rpcError?.message}`);
            setLoading(false);
            setConfirmModal(prev => ({ ...prev, isOpen: false }));
            return;
          }
  ...
  // Universal Fee Increment with Target Year and Scope Mode
  function handleUniversalFeeIncrease(e: React.FormEvent) {
    e.preventDefault();
    const val = parseFloat(incValue);
    if (!val || val <= 0) return;

    const modeText = scopeMode === 'permanent' 
      ? 'oi bochor theke shuru kore poroborti shob bochorer jonno permanent-bhabe' 
      : 'shudhumatro oi nirdishto bochorer jonno';

    setConfirmModal({
      isOpen: true,
      title: 'Shobar Fee Briddhi Nishchitkoron',
      description: `Apni ki nishchit je ${selectedTargetYear} sal-er (jara age paid korechen tader shoho shobar) fee ${incType === 'fixed' ? `₹${val} taka` : `${val}%`} briddhi korte chan? Scope: ${modeText}।`,
      action: async () => {
        setLoading(true);
        setMsg('');

        const { error: rpcErr } = await supabase.rpc('increase_all_fees', {
          inc_type: incType,
          inc_val: val,
        ...
        if (rpcErr) {
          try {
            const targetFees = allFees.filter(f => selectedTargetYear === 'all' || f.year_cycle === selectedTargetYear);
            for (const f of targetFees) {
            ...
            if (scopeMode === 'permanent') {
              for (const fam of families) {
                let base = Number(fam.base_monthly_fee || 100);
                let newBase = incType === 'fixed' ? base + val : Math.round(base + (base * (val / 100)));
                await supabase.from('families').update({ base_monthly_fee: newBase }).eq('id', fam.id);
              }
            }
          } catch (err: any) {
            setMsg('Fee briddhi korte shomoshya hoyeche.');
            setLoading(false);
            setConfirmModal(prev => ({ ...prev, isOpen: false }));
            return;
          }
        ...
        setIncValue('');
        setConfirmModal(prev => ({ ...prev, isOpen: false }));
        setMsg(`Shofolbhabe ${selectedTargetYear} sal-er shobar fee ${incType === 'fixed' ? `₹${val} taka` : `${val}%`} barano hoyeche!`);
        await loadAllData();
      }
    });
  }

  // Notices, Funds & Profile
  async function handlePublishNotice(e: React.FormEvent) {
    ...
    setMsg('Notish prokashito hoyeche!');
    setNoticeTitle('');
    setNoticeContent('');
    setLoading(false);
    loadAllData();
  ...
  async function handleDeleteNotice(id: string) {
    if (!confirm('Notish muche phelte chan?')) return;
  ...
  async function handleAddTransaction(e: React.FormEvent) {
    ...
    setMsg('Fund hishab shongrokkhito hoyeche!');
    setTransCategory('');
    setTransAmount('');
    setTransDesc('');
    setLoading(false);
  ...
  async function handleSaveMosqueProfile(e: React.FormEvent) {
    ...
    if (!res?.error) {
      setMsg('Masjid profile o letterhead shofolbhabe update hoyeche!');
      loadAllData();
    } else {
      setMsg('Profile update korte shomoshya hoyeche.');
    }
    setLoading(false);
  }
      ...
      <header className="bg-emerald-900 text-white p-4 shadow-lg sticky top-0 z-50">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-lg md:text-xl font-black">{mosqueProfile.name || 'অ্যাডমিন কন্ট্রোল প্যানেল'}</h1>
      ...
      <main className="max-w-4xl mx-auto p-4 flex flex-col gap-6">
        {msg && (
          <div className={`p-3 rounded-xl text-xs font-semibold text-center border ${
            msg.includes('shomoshya') || msg.includes('সমস্যা') || msg.includes('failed') ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
          }`}>
            {msg}
          </div>
        )}
        ...
        {activeTab === 'fees' && (
          <div className="flex flex-col gap-5">
            {/* Universal Fee Increase with Scope & Year Selection */}
            <section className="bg-gradient-to-r from-emerald-800 to-teal-800 text-white p-5 rounded-2xl shadow-sm">
              <h2 className="text-sm font-bold flex items-center gap-2 mb-1">
                ...
                ইউনিভার্সাল ফি বৃদ্ধি (সবার ফি একসাথে সমহারে বৃদ্ধি)
              </h2>
              <p className="text-xs text-emerald-200 mb-3">
                যাঁরা আগে পেইড করে ফেলেছেন তাঁদের ক্ষেত্রেও বর্ধিত অংশ স্বয়ংক্রিয়ভাবে বকেয়া হিসেবে যুক্ত হবে।
              </p>

              <form onSubmit={handleUniversalFeeIncrease} className="flex flex-col gap-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2.5">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-emerald-200 font-bold mb-1">কার্যকর বছর:</span>
                    <select
                      value={selectedTargetYear}
                      onChange={(e) => setSelectedTargetYear(e.target.value)}
                      className="bg-emerald-950/80 text-amber-300 font-bold text-xs p-2.5 rounded-xl border border-white/20 outline-none">
                      <option value="১৪৩১-১৪৩২">১৪৩১-১৪৩২ বঙ্গাব্দ</option>
                      <option value="১৪৩২-১৪৩৩">১৪৩২-১৪৩৩ বঙ্গাব্দ</option>
                      <option value="১৪৩৩-১৪৩৪">১৪৩৩-১৪৩৪ বঙ্গাব্দ</option>
                      <option value="all">সকল বছর (All)</option>
                    </select>
                  </div>

                  <div className="flex flex-col">
                    <span className="text-[10px] text-emerald-200 font-bold mb-1">বৃদ্ধির আওতা:</span>
                    <select
                      value={scopeMode}
                      onChange={(e: any) => setScopeMode(e.target.value)}
                      className="bg-emerald-950/80 text-white font-bold text-xs p-2.5 rounded-xl border border-white/20 outline-none">
                      <option value="permanent">পরবর্তী সব বছর (স্থায়ী)</option>
                      <option value="single_year">শুধুমাত্র নির্বাচিত বছরের জন্য</option>
                    </select>
                  </div>

                  <div className="flex flex-col">
                    <span className="text-[10px] text-emerald-200 font-bold mb-1">বৃদ্ধির ধরণ:</span>
                    <div className="flex bg-emerald-950/50 p-1 rounded-xl border border-white/20 text-xs h-[38px]">
                      <button
                        type="button"
                        onClick={() => setIncType('fixed')}
                        className={`flex-1 rounded-lg font-bold transition ${incType === 'fixed' ? 'bg-amber-400 text-slate-900' : 'text-emerald-200'}`}>
                        + টাকা (₹)
                      </button>
                      <button
                        type="button"
                        onClick={() => setIncType('percent')}
                        className={`flex-1 rounded-lg font-bold transition ${incType === 'percent' ? 'bg-amber-400 text-slate-900' : 'text-emerald-200'}`}>
                        + শতাংশ (%)
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] text-emerald-200 font-bold mb-1">পরিমাণ:</span>
                    <div className="flex gap-2">
                      <input
                        type="number"
                        required
                        placeholder={incType === 'fixed' ? 'যেমন: ৫০' : 'যেমন: ৫%'}
                        value={incValue}
                        onChange={(e) => setIncValue(e.target.value)}
                        className="bg-white text-slate-900 text-sm font-bold px-3 py-2 rounded-xl outline-none w-full"
                      />
                      <button
                        type="submit"
                        disabled={loading}
                        className="bg-amber-400 hover:bg-amber-300 text-slate-900 font-black text-xs px-4 rounded-xl transition shadow shrink-0">
                        প্রয়োগ
                      </button>
                    </div>
                  </div>
            ...
            <div className="bg-white p-3 rounded-2xl border border-slate-200 flex items-center gap-2 shadow-sm">
              ...
              <input
                type="text"
                placeholder="নাম, ফোন বা পরিচয় নম্বর দিয়ে খুঁজুন..."
                value={searchQuery}
        ...
        {/* Tab 5: Mosque Profile & Letterhead */}
        {activeTab === 'settings' && (
          <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h2 className="text-base font-bold flex items-center gap-2 mb-4 text-slate-800">
