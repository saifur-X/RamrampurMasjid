...
export default function AdminDashboard() {
  ...
  // Manually Paid ONLY for the single family who pays
  ...
  function handleUniversalFeeIncrease(e: React.FormEvent) {
    ...
    setConfirmModal({
      ...
      description: `Apni ki nishchit je shokol poribar-er (jaro ja base fee ache) bokea mashik fee ${incType === 'fixed' ? `₹${val} taka` : `${val}%`} briddhi korte chan?`,
      action: async () => {
        setLoading(true);
        ...
        if (error) {
          setMsg('Fee briddhi korte shomoshya hoyeche. SQL run kora ache kina dekhun.');
        } else {
          setIncValue('');
          setMsg(`Shofolbhabe shokol poribar-er fee ${incType === 'fixed' ? `₹${val} taka` : `${val}%`} barano hoyeche!`);
          loadAllData();
        }
      }
    });
      ...
      <main className="max-w-4xl mx-auto p-4 flex flex-col gap-6">
        {msg && (
          <div className={`p-3 rounded-xl text-xs font-semibold text-center border ${
            msg.includes('shomoshya') || msg.includes('সমস্যা') ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
          }`}>
            {msg}
          </div>
        )}
          ...
          <div className="flex flex-col gap-5">
            ...
            <section className="bg-gradient-to-r from-emerald-800 to-teal-800 text-white p-5 rounded-2xl shadow-sm">
              <h2 className="text-sm font-bold flex items-center gap-2 mb-1">
                ...
                ইউনিভার্সাল ফি বৃদ্ধি (সবার ফি একসাথে সমহারে বাড়ান)
              </h2>
